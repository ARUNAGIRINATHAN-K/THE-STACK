...
- [Okto Nexus](https://github.com/OktoLabsAI/okto-nexus) `🔬` `[Python]` `[MCP]` - Local-first coordination layer for teams running multiple CLI coding agents; single-winner task claims stop two agents from starting the same work, human-approval queues hold risky actions with the diff and reasoning attached, and durable handoff history lets a killed session's replacement resume without re-explaining what already happened.
